...
/**
 * Control surface through which an {@link Activity} attaches to a {@link
 * io.flutter.embedding.engine.FlutterEngine}.
 *
 * <p>An {@link Activity} that contains a {@link
 ...
 *
 * <ol>
 *   <li>Once an {@link Activity} is created, and its associated {@link
 *       io.flutter.embedding.engine.FlutterEngine} is executing Dart code, the {@link
 *       Activity} should invoke {@link #attachToActivity( ExclusiveAppComponent,
 *       Lifecycle)}. At this point the {@link io.flutter.embedding.engine.FlutterEngine} is
 *       considered "attached" to the {@link Activity} and all {@link ActivityAware}
 *       plugins are given access to the {@link Activity}.
 *   <li>Just before an attached {@link Activity} is destroyed for configuration change
 *       purposes, that {@link Activity} should invoke {@link
 *       #detachFromActivityForConfigChanges()}, giving each {@link ActivityAware} plugin an
 *       opportunity to clean up its references before the {@link Activity is
 *       destroyed}.
 *   <li>When an {@link Activity} is destroyed for non-configuration-change purposes, or
 *       when the {@link Activity} is no longer interested in displaying a {@link
 *       io.flutter.embedding.engine.FlutterEngine}'s content, the {@link Activity}
 *       should invoke {@link #detachFromActivity()}.
 *   <li>When a {@link Activity} is being attached while an existing {@link
 ...
 * </ol>
 *
 * The attached {@link Activity} should also forward all {@link Activity}
...
public interface ActivityControlSurface {
   ...
   * ExclusiveAppComponent}'s {@link Activity}.
   */
   ...
   * Activity} is about to be destroyed due to configuration changes.
   *
   ...
   * Call this method from the {@link Activity} that is attached to this {@code
   * ActivityControlSurfaces}'s {@link io.flutter.embedding.engine.FlutterEngine} when the {@link
   * Activity} is about to be destroyed for non-configuration-change reasons.
   *
   * <p>This method gives each {@link ActivityAware} plugin an opportunity to clean up its
   * references before the {@link Activity is destroyed}.
   */
  ...
  /**
   * Call this method from the {@link Activity} or {@code Fragment} that is attached to
   * this {@code ActivityControlSurface}'s {@link io.flutter.embedding.engine.FlutterEngine} when
   * the associated method is invoked in the {@link Activity} or {@code Fragment}.
   */
   ...
   * Call this method from the {@link Activity} or {@code Fragment} that is attached to
   * this {@code ActivityControlSurface}'s {@link io.flutter.embedding.engine.FlutterEngine} when
   * {@link Activity#onCreate(Bundle)} or {@code Fragment#onCreate(Bundle)} is invoked
